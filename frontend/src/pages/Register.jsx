...
const Register = () => {
  ...
  const { roles, loading, error } = useSelector((state) => state.auth);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(registerUserThunk(payload)).unwrap();
      ...
      >
        ...
        <div className="flex-1 p-6 md:p-12 flex flex-col justify-center bg-white/10">
          ...
          <div className="mb-10">
            ...
            <h2 className="text-3xl font-bold text-slate-800 mt-1 mb-1 tracking-tight">
              Create Account
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button
              whileHover={{
                ...
                boxShadow:
                  "0 10px 20px -5px rgba(99, 102, 241, 0.6)"
              }}
              whileTap={{ scale: 0.98 }}
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-[#818cf8] to-[#a78bfa] text-white font-bold py-3 rounded-lg mt-6 transition-all text-lg shadow-lg shadow-indigo-100 disabled:opacity-60"
            >
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <p className="text-white/90 text-sm leading-relaxed font-medium italic">
                    {onboardingSteps[activeSlide].desc}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>
